...
/**
 * 
 * Making the change the getFacetedData function on line: 291 for checking the applied filters list.
 * filterapplied,getfilterapplied are the additional custom functions.
 */
    ...
    implements \Amasty\Shopby\Api\Data\FromToFilterInterface
{
    ...
    private function getFacetedData()
    {
        if(is_null($this->facetedData)) {
            ...
            if (in_array($attribute->getAttributeCode(),$this->getfilterapplied())) {       //Custom change made for checking whether the filter is applied or not
